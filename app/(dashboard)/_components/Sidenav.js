  ...
  return (
    ...
    <section>
      ...
      {menuList.map((item, index)=> (
        <button className={`flex gap-2 p-2 mt-2 items-center rounded-md hover:bg-zinc-800 w-full text-slate-300 ${activeIndex==index?'bg-zinc-700':null}`} onClick={()=>setActiveIndex(index)}><item.icon/><h2>{item.name}</h2></button>
      ))}
